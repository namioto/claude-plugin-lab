...
servicedeskapi는 추가 헤더 `X-ExperimentalApi: opt-in`이 필요합니다. URL은 항상 환경변수 `$ATLASSIAN_URL` 값을 사용합니다. 프로젝트 키는 환경변수 `$JSM_PROJECT` 값을 사용합니다. 후행 슬래시(`/`)가 있으면 제거 후 사용합니다.
...
- 서비스데스크 메타: `$JSM_PROJECT`와 일치하는 `serviceDeskId`, 프로젝트명
...
2. `$JSM_PROJECT`와 일치하는 항목의 `serviceDeskId` 확인
