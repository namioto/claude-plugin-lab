...
4. **API 키 설정**
   ...
   "이제 Atlassian 연동을 위한 인증 정보를 설정하겠습니다. 입력하신 값은 Claude Code의 보안 설정 파일(~/.claude/settings.local.json)에 저장되며, 외부로 유출되지 않습니다."
   ...
   입력받은 뒤, Read 도구로 `~/.claude/settings.local.json`을 읽습니다:
   - 파일이 존재하면: 기존 JSON을 파싱하여 `env` 항목에 `JIRA_EMAIL`과 `JIRA_API_KEY`를 추가하거나 덮어씁니다. 나머지 기존 설정은 반드시 보존합니다.
   - 파일이 없으면: 아래 구조로 새로 생성합니다.

   Write 도구로 `~/.claude/settings.local.json`에 저장합니다:
   ```json
   {
     "env": {
       "JIRA_EMAIL": "[입력값]",
...
**주의사항:**
- API 키와 이메일은 `~/.claude/settings.local.json`의 `env` 항목에만 저장합니다. `.claude/alfred.local.md`에는 절대 기재하지 않습니다.
- `~/.claude/settings.local.json` 저장 시 기존에 존재하던 다른 설정값은 절대 삭제하거나 덮어쓰지 않습니다.
